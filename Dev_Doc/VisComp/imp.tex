...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{images/interact/interact_step}
	%\includegraphics[width=.45\linewidth]{images/interact/02.png}
	%\includegraphics[width=.45\linewidth]{images/interact/03.png}
	%\includegraphics[width=.3\linewidth]{images/interact/04.png}
	%\includegraphics[width=.3\linewidth]{images/interact/05.png}
	%\includegraphics[width=.45\linewidth]{images/interact/06.png}
	\caption{\mdf{The procedure of interaction:
		(a) Load all the input point sets into the system. These point sets records the same scene at different time. The objects inside the scene have been moved. (b) Pick one point set to add boxes to indicate layout for objects. The box in white is the box currently under editing. (c) Add boxes to represent each object inside the scene. One color represents one object. The interaction allows multiple boxes to represent same object. (e.g. the desk is represented by three boxes in same color) (d) The interaction is finished.}}
\end{figure}
...
%
This means that the object models are vertically arranged in latent space as shown in \mdf{Figure~\ref{fig:localoptimal}(b)}. 
%
...
\begin{figure}
	...
	\caption{\label{fig:localoptimal} An example result when our algorithm converges to a local optimal. \mdf{(a) shows the  segmentation result in six point sets. The algorithm stuck at a local optimal where a part of the table and a part of the chair are segmented into one combined object. (b) shows the same result as (a), but in the view of Gaussian centroids. It shows three group of Gaussian centroids vertically arranged. Each row shows one group of Gaussian centroids representing one object. It shows that the chair and the table are not perfectly segmented.}}
\end{figure}
