...
%
Another way is to automatically \mdf{compose a scene model from an image based on existing 3D shape models~\cite{Liu2015Model,Chen:2014:ASM:2661229.2661239}.}  
...
However, there is a large gap between the desired 3D model dataset and current available scene capturing tools. Typically, clean, complete and separated models for objects are desired to construct \mdf{a} scene database. 
...
\cxj{need discussion here for more accurate and clear expression.}
%
...
\begin{enumerate}
	...
	\item We design an \mdf{interactive} tool for joint registration and co-segmentation based on the generative model. 
\end{enumerate}