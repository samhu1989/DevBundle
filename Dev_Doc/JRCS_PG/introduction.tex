...
%
\mdf{In both schemes, it is non-trivial to recover the correspondences of objects in different point sets due to the occlusions by human bodies in the first scheme or sparse sampling on times in the second scheme. 
In the first scheme, additional challenging processing may be also required such as tracking objects with severe occlusions by human bodies.}
%The main challenge brought in by the second scheme is that we may not be able to solve the object correspondence by a local search due to the sparse sampling over time. However, the very same challenge exists in the first scheme due to the occlusion caused by human bodies, not to mention other additional process (e.g. tracking with severe occlusions) needed for human bodies.
...
%
\mdf{This makes the big difference from non-rigid registration problems where point motions are smooth everywhere.}
