...
%
For each dataset, we model a 3D scene using object models from 3D Warehouse~\cxj{add reference.} 
We convert the mesh model of the scene into a point set using the Poisson sampling method~\cite{PossionSampling}. \cxj{reference for possion sampling.}
%
Then we manually move the objects according to their functions and generate multiple point sets. 


To collect data of real scenes, we scan a scene at different times using \cxj{voxel hashing or kinect fusion?}  \cite{KinectFusion}.
%
...
\begin{enumerate}
	...
	\item Turn the meshes into point sets
	 with sampling method of \cite{PossionSampling}
...
\cxj{My suggestion for algorithm names: JRCS-Basic and JRCS-Bilateral. I also suggest move this section before co-segmentation.  }
\begin{table}
	\centering
...
\cxj{Why Table 4? New group of experiments? }
%
...
\cxj{Add more discussion about the results. Discuss Fig 10.} 
...
\cxj{Does Poisson sampling lead to evenly distributed points?}
%
...
%
\mdf{To test this, we pick a group of point sets, and} gradually remove certain percentage ( $0\%-30\%$ ) of points from each point set \cxj{not evenly, but on some specific part?}.
\cxj{For different group of point sets, do you use the same interaction input?}
% 
...
\cxj{Do not use 'the' before "Figure".}
...
\end{enumerate}
\cxj{Do you use the same group of data for Figure~\ref{fig:incompleteness}?}
%
...
\cxj{Do not use "row 09" to indicate the subfigure.}
...
\cxj{You said you use "Kinect fusion" at the beginning.}
%
